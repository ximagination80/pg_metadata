    ...
    if (cfg.debug) {
      *("=" * 150)
      e.foreach(table => {
        ...
        *()
        **(templateColumns, "#", "Primary Key", "Column Name", "Database Type", "AutoGenerated Primary Key", "Nullable")
        ...
        *()
        *("Columns limits")
        **(templateColumnsDetails, "#", "Column Name", "Default value", "Char max len", "Char octet len", "Numeric prec.", "Numeric prec. radix", "Numeric scale", "Datetime prec.")
        table.column.zipWithIndex.foreach { case (c, n) =>
